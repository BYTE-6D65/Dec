    ...
    const savePost = async () => {
        ...
        try {
            const response = await fetch('/api/blog/save', {
                ...
                body: JSON.stringify({
                    id: selectedPostId(), // Pass ID if editing existing post
                    title: title(),
                    date: date(),
                    preview: preview(),
                    slug: slug(),
            ...
            setMessage(`Post saved as ${result.filename} `);

            // Invalidate blog posts cache to refresh the list
            await revalidate("blog-posts");

            // If we just created a new post, select it (optional, but good UX)
            // For now, we'll just keep the form as is so user can continue editing
            ...
            left={
                <div class="h-full w-full border-r border-border flex flex-col bg-surface">
                    ...
                    <div class="flex-1 overflow-y-auto px-0 py-0">
                        <Show when={posts()} fallback={<div class="text-text-muted">Loading posts...</div>}>
                ...
                <div class="h-full w-full flex flex-col bg-background p-8 overflow-y-auto">
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <div class="flex items-center gap-4">
                            {message() && (
                                <span class={`text - sm font - mono ${message().startsWith('✓') ? 'text-accent' : 'text-error'} `}>
                                    {message()}
                                </span>
                            )}
                            <button
